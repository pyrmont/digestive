...
(defn digest
  ...
  ```
  [input]
  # SHA-3-512 parameters
  (def rate 72) # r = 576 bits / 8 = 72 bytes
  (def output-len 64) # 512 bits / 8 = 64 bytes
  ...
  (while (< pos (length P))
    ...
    (set S (keccak-f S))
    (set pos (+ pos rate)))
  # Squeezing phase: extract first 512 bits
